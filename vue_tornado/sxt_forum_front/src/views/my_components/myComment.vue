    ...
    <div class="topics_content">
      <div class="mui-scroll-wrapper mui-scroll_content">
        <div class="mui-scroll">
          <div class="comment_content" v-for="comment in comments" :key="comment.id">
            ...
            <div class="commenter_right">
              <div class="commenter_nickname">
                ...
                <div
                  ...
                  @click.prevent="deleteMyComment(comment.id)"
                ></div>
              </div>
              <div class="commenter_content">{{comment.content}}</div>
...
export default {
  ...
  methods: {
    async deleteMyComment(comment_id) {
      ...
      var result = await myaxios("POST", deleteMyCommentURL, obj1);
      if (result.data.code  == 200) {
        Toast("删除成功！");
        var obj2 = {
          userID: this.user._id
        };
        var result2 = await myaxios("POST", getMyCommentURL, obj2);
  ...
  async created() {
    ...
    if (result1.data.user) {
      ...
      var result2 = await myaxios("POST", getMyCommentURL, obj);
      this.comments = result2.data.comments;
    }
    else{
        this.$router.push("/my");
