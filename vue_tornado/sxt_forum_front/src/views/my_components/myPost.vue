          ...
          <ul class="topics_content_ul">
            <li v-for="item in topics" :key="item.id">
              <div class="topic_left">
                <div v-if="item.imgs">
                <img :src="'http://localhost:8000'+item.imgs[0]" alt />
                </div>
              </div>
              <router-link :to="'/topic/detail/'+item.id" tag="span">
                <div class="topic_right">
                  <div class="topic_poster_nickname">
                    {{item.user.nick_name}}
                    <div
                      class="mui-icon mui-icon-closeempty"
                      @click.prevent="deleteMyTopic(item.id)"
                    ></div>
                  </div>
...
export default {
  ...
  methods: {
    async deleteMyTopic(topicID) {
      ...
      var result = await myaxios("POST", deleteMyTopicURL, obj);
